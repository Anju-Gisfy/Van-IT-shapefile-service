...
namespace ShapefileService
{
    public partial class Service1 : ServiceBase
    {
        ...
        public void test()
        {
            ...
            string conn = "server=3.7.34.230;database=VanIT;User ID=sa;Password=pass@123;";
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            for (int j = 0; j < dt.Rows.Count; j++)
                            {
                                ...
                                try
                                {
                                    ...
                                    //to check new layer create or not
                                    int layercount = 0;
                                    //checking for layer name in division table
                                    ...
                                    try
                                    {
                                        ...
                                        nvc.Clear();
                                        nvc.Add("@StatementType", "layers");
                                        nvc.Add("@operation", "select");
                                        nvc.Add("@divisionname", dtn.Rows[0]["namedivision"].ToString());

                                        //nvc.Add("@UserName", UserName);
                                        dtlayer = new clsConnnection().fnExecuteProcedureSelectWithCondtion("[VanIT].[dbo].[sp_shapefile_import]", nvc);
                                    }
                                    catch (Exception ex) { }
                                    ...
                                    using (dtn)
                                    {
                                        ...
                                        if (dtn.Rows.Count > 0)
                                        {
                                            for (int x = 0; x < dtn.Rows.Count; x++)
                                            {
                                                try
                                                {
                                                    ...
                                                    nvc.Add("@divisionname", dtn.Rows[x]["namedivision"].ToString());
                                                    nvc.Add("@createby", dt.Rows[x]["user_id"].ToString());
                                                ...
                                                try
                                                {
                                                    ...
                                                    if ((dt.Rows[j]["is_delete"].ToString() == "true") || (dtss.Rows.Count > 0))
                                                    {
                                                        try
                                                        {
                                                            ...
                                                            clscon.objCom.Parameters.AddWithValue("@divisionname", dtn.Rows[x]["namedivision"].ToString());
                                        ...
                                        try
                                        {
                                            ...
                                            string commandn = "curl -v -u admin:geoserver -XPUT -H \"Content-type: text/xml\" -d \"<layer><defaultStyle><name>" + stylename + "</name></defaultStyle></layer>\" \"http://3.7.34.230:8080/geoserver/rest/layers/cite:" + viewname + "\"";
                                        ...
                                        //layer name insertion into tbldivision
                                        string layersname = "";
                                        if (layercount > 0)
                                        {
                                            ...
                                            try
                                            {
                                                ...
                                                //SqlCommand cmdn = new SqlCommand("sp_shapefile_queue", con);
                                                clscon.objCom.CommandType = CommandType.StoredProcedure;
                                                clscon.objCom.Parameters.AddWithValue("@StatementType", "layers");
                                                clscon.objCom.Parameters.AddWithValue("@operation", "update");
                                                clscon.objCom.Parameters.AddWithValue("@divisionname", dtn.Rows[0]["namedivision"].ToString());
                                                clscon.objCom.Parameters.AddWithValue("@layername", layersname);
                                                ...
                                                WriteToFile("layers updated in tbldivision at " + DateTime.Now);
                                        ...
                                        try
                                        {
                                            ...
                                            clscon.objCom.Parameters.AddWithValue("@shapefileid", dt.Rows[j]["id"]);
                                            clscon.fnOpenConnection();
                                            int i = clscon.objCom.ExecuteNonQuery();
                                            clscon.fnCloseConnection();
                                            if (i > 0)
